...
# เลือกผลการเทรนที่จะใช้: --run runs/img96  (ไม่ใส่ = model.pt/norm_stats.json ที่วางไว้ข้างสคริปต์นี้)
RUN_DIR = SCRIPT_DIR
if "--run" in sys.argv:
    _i = sys.argv.index("--run")
    RUN_DIR = os.path.join(SCRIPT_DIR, sys.argv[_i + 1])
    del sys.argv[_i:_i + 2]
MODEL_PATH = os.path.join(RUN_DIR, "model.pt")
NORM_STATS_PATH = os.path.join(RUN_DIR, "norm_stats.json")
...
USE_MAXPOOL = _norm_stats.get('use_maxpool', True)  # ไฟล์เก่าไม่มี key นี้ = ใช้ maxpool (แบบ round 3)

# auto-detect device: ลอง DirectML ก่อน แล้วค่อย cuda แล้วค่อย cpu (เหมือนใน TrainingCNN.py)
try:
