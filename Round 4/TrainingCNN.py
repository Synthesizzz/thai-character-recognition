...
def main():
    ...
    print(f"RUN: {RUN_NAME} (IMG_SIZE={IMG_SIZE}, maxpool={'on' if USE_MAXPOOL else 'off'}, optimizer={OPTIMIZER})")
    ...
    def save_metrics(finished):
        ...
        with open(metrics_path, 'w', encoding='utf-8') as mf:
            json.dump({
                'run': RUN_NAME, 'seed': SEED, 'img_size': IMG_SIZE, 'use_maxpool': USE_MAXPOOL, 'optimizer': OPTIMIZER,
